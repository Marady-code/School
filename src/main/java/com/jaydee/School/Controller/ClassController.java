...
public class ClassController {
	...
	public ResponseEntity<?> createClass(@Valid @RequestBody ClassDTO classDTO) {
		try {
			return ResponseEntity.status(HttpStatus.CREATED).body(classService.create(classDTO));
		} catch (Exception e) {
	...
	public ResponseEntity<?> deleteClass(@PathVariable Long id) {
		try {
			...
			return ResponseEntity.ok().body(Collections.singletonMap("message", "Class deleted successfully"));
		} catch (ResourceNotFound e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Collections.singletonMap("error", e.getMessage()));
		}
	}
	...
	@PreAuthorize("hasRole('ADMIN')")
	public ResponseEntity<?> addStudentToClass(@PathVariable Long classId, @PathVariable Long studentId) {
		try {
